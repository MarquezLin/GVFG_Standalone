...
namespace
{
    ...
    constexpr DWORD kVideoFormatChangeTimeoutMs = 100;
    ...
    {
        return format == GVFG_PIXFMT_Y210 ? 10u : format == GVFG_PIXFMT_YUY2 ? 8u
                                                                             : 0u;
    }
...
namespace gvfg::internal
{
    ...
    gvfg_status_t GigabyteCaptureSession::open_device_index(size_t deviceIndex)
    {
        ...
        if (deviceIndex >= devices.size())
            return reject(GVFG_ENODEV, "Capture backend open rejected: device index is unavailable");
    ...
    {
        if (devicePath.empty())
            return reject(GVFG_EINVAL, "Capture backend open rejected: device selection is invalid");
        close();
    ...
    gvfg_status_t GigabyteCaptureSession::ensure_vendor_channel_open()
    {
        ...
        if (device_path_.empty())
            return reject(GVFG_ESTATE, "Capture backend channel open rejected: no device is selected");
        ...
        if (device_handle_ == INVALID_HANDLE_VALUE)
            return reject(GVFG_ENODEV, "Capture device open failed");
        ...
        if (!stop_event_)
        {
            ...
            return reject(GVFG_EIO, "Capture cancellation event creation failed");
        }
        recovery_event_ = CreateEventW(nullptr, TRUE, FALSE, nullptr);
        if (!recovery_event_)
        {
            close_vendor_channel();
            return reject(GVFG_EIO, "Capture recovery event creation failed");
        }
    ...
    void GigabyteCaptureSession::close_vendor_channel()
    {
        ...
        channel_open_ = false;
        {
            ...
            format_change_pending_ = false;
            requested_video_format_ = GVFG_PIXFMT_UNKNOWN;
    ...
    {
        if (channel > 1)
            return reject(GVFG_EINVAL, "Capture backend channel index is invalid");
        if (channel_open_)
            return reject(GVFG_ESTATE, "Capture backend channel is already open");
        channel_ = channel;
        return GVFG_OK;
    }
    ...
    gvfg_status_t GigabyteCaptureSession::set_zero_copy_enabled(bool enabled)
    {
        if (channel_open_ || running_)
            return reject(GVFG_ESTATE, "Capture frame memory mode is already fixed");
    ...
    {
        if (running_)
            return reject(GVFG_ESTATE, "Capture audio mode cannot change while running");
    ...
    gvfg_status_t GigabyteCaptureSession::set_video_format(gvfg_pixel_format_t format)
    {
        ...
        gvfg_pixel_format_t requestedFormat = GVFG_PIXFMT_UNKNOWN;
        {
            std::lock_guard<std::mutex> lock(state_mutex_);
            requestedFormat = requested_video_format_;
        }

        if (requestedFormat == format)
            return GVFG_OK;

        if (!events_.hVideoFormatChangedEvent)
            return reject(GVFG_EIO, "Capture backend did not provide a video-format event");
        if (!ResetEvent(events_.hVideoFormatChangedEvent))
            return reject(GVFG_EIO, "Capture video-format event reset failed");
    ...
    {
        if (!context_)
            return reject(GVFG_ESTATE, "Capture video info requested before open");
        GVFG_VIDEO_INFO info{};
        gvfg_status_t status = GVFG_OK;
        {
            std::lock_guard<std::mutex> captureLock(capture_mutex_);
            status = from_vendor(GvfgGetVideoInfo(context_, channel_, &info), "GvfgGetVideoInfo");
        }
        if (status != GVFG_OK)
            return status;
    ...
    gvfg_status_t GigabyteCaptureSession::get_signal_status(gvfg_signal_status_t &out)
    {
        ...
        if (openStatus != GVFG_OK)
            return openStatus;
        bool formatChangePending = false;
        gvfg_pixel_format_t requestedFormat = GVFG_PIXFMT_UNKNOWN;
        {
            std::lock_guard<std::mutex> lock(state_mutex_);
            formatChangePending = format_change_pending_;
            requestedFormat = requested_video_format_;
        }
        ...
        if (formatChangePending)
        {
            if (!events_.hVideoFormatChangedEvent)
                return reject(GVFG_EIO, "Capture backend did not provide a video-format event");
            const DWORD wait = WaitForSingleObject(events_.hVideoFormatChangedEvent,
                                                   kVideoFormatChangeTimeoutMs);
            if (wait != WAIT_OBJECT_0 && wait != WAIT_TIMEOUT)
                return reject(GVFG_EIO, "Capture video-format event wait failed");

            // Setting the already-active color depth does not necessarily emit a
            // format-change event.  Query once after the wait and accept the
            // request when the hardware is already reporting the requested format.
            ...
            bool formatMatches = false;
            {
                std::lock_guard<std::mutex> lock(state_mutex_);
                format_change_pending_ = false;
                formatMatches = cached_signal_.pixel_format == requestedFormat;
                out = cached_signal_;
            }
            if (!formatMatches)
                return reject(GVFG_ETIMEOUT,
                              "Capture video format did not match the requested format");
            return GVFG_OK;
        }
    ...
    {
        if (running_)
            return reject(GVFG_ESTATE, "Capture stream is already running");
        const gvfg_status_t openStatus = ensure_vendor_channel_open();
        if (openStatus != GVFG_OK)
            return openStatus;
        const gvfg_status_t resetStatus = from_vendor(GvfgResetEvents(&events_), "GvfgResetEvents");
        if (resetStatus != GVFG_OK)
            return resetStatus;
        configured_ = true;
        return GVFG_OK;
    }
    ...
    gvfg_status_t GigabyteCaptureSession::start_stream()
    {
        if (running_)
            return GVFG_OK;
        if (!configured_)
            return reject(GVFG_ESTATE, "Capture stream has not been configured");
    ...
    gvfg_status_t GigabyteCaptureSession::stop_stream()
    {
        ...
        if (wasRunning && channel_open_ && context_)
        {
            ...
            {
                const gvfg_status_t resetStatus = from_vendor(GvfgResetEvents(&events_),
                                                              "GvfgResetEvents");
                if (result == GVFG_OK)
                    result = resetStatus;
            }
    ...
    gvfg_status_t GigabyteCaptureSession::wait_frame(uint32_t timeoutMs, gvfg_frame_t &out)
    {
        ...
        if (!running_ || !context_)
            return reject(GVFG_ESTATE, "Capture frame read rejected: stream is not running");
        {
            std::lock_guard<std::mutex> lock(state_mutex_);
            if (frame_held_)
                return reject(GVFG_ESTATE, "Capture frame read rejected: previous frame is held");
        }
        ...
        if (!handles[2])
            return reject(GVFG_EIO, "Capture backend did not provide a video-frame event");
        ...
        if (wait != WAIT_OBJECT_0 + 2 && wait != WAIT_OBJECT_0 + 3)
            return reject(GVFG_EIO, "Capture video event wait failed");
        {
            std::lock_guard<std::mutex> lock(state_mutex_);
        ...
        if (status != GVFG_OK)
            return status;
        if (!data || bytes == 0)
            return reject(GVFG_EIO, "Capture backend returned an invalid video frame");
    ...
    {
        {
            std::lock_guard<std::mutex> lock(state_mutex_);
            if (!frame_held_)
                return reject(GVFG_ESTATE, "Capture frame release rejected: no frame is held");
        ...
        uint32_t timeoutMs, void *destination, uint32_t destinationCapacity, uint32_t &outBytes)
    {
        ...
        if (!running_ || !audio_enabled_ || !destination)
            return reject(GVFG_ESTATE, "Capture audio read rejected");
        ...
        if (frameBytes == 0 || frameBytes > destinationCapacity)
            return reject(GVFG_EINVAL, "Capture audio destination is too small");
        if (!events_.hAudioFrameInEvent)
            return reject(GVFG_EIO, "Capture backend did not provide an audio-frame event");
        ...
        if (wait == WAIT_OBJECT_0 + 1)
            return wait_for_recovery(timeoutMs);
        if (wait != WAIT_OBJECT_0 + 2 && wait != WAIT_OBJECT_0 + 3)
            return reject(GVFG_EIO, "Capture audio event wait failed");
        {
            std::lock_guard<std::mutex> lock(state_mutex_);
            if (wait == WAIT_OBJECT_0 + 2)
                ++audio_event_wakes_;
    ...
    void GigabyteCaptureSession::fill_debug_stats(gvfg_debug_backend_stats_t &out) const
    {
        ...
        const ChannelErrorState::VendorDetail vendorDetail = error_state_.vendor_detail();
        out.last_vendor_error_code = vendorDetail.code;
        strncpy_s(out.last_vendor_api, vendorDetail.api.c_str(), _TRUNCATE);
        strncpy_s(out.last_vendor_error_name, vendorDetail.name.c_str(), _TRUNCATE);
    }
    ...
    gvfg_status_t GigabyteCaptureSession::from_vendor(GVFG_HRESULT result, const char *apiName)
    {
        if (result == GVFG_HRESULT_OK)
            return GVFG_OK;

        const char *name = "GVFG_HRESULT_UNKNOWN";
        gvfg_status_t status = GVFG_EIO;
        switch (result)
        {
        case GVFG_HRESULT_ERROR:
            name = "GVFG_HRESULT_ERROR";
            break;
        case GVFG_HRESULT_DEV_ERROR:
            name = "GVFG_HRESULT_DEV_ERROR";
            break;
        case GVFG_HRESULT_DEV_BUSY:
            name = "GVFG_HRESULT_DEV_BUSY";
            status = GVFG_EBUSY;
            break;
        case GVFG_HRESULT_VIDEO_CHN_ERROR:
            name = "GVFG_HRESULT_VIDEO_CHN_ERROR";
            break;
        case GVFG_HRESULT_CONTEXT_ERROR:
            name = "GVFG_HRESULT_CONTEXT_ERROR";
            status = GVFG_ESTATE;
            break;
        case GVFG_HRESULT_VIDEO_CHN_INVALID:
            name = "GVFG_HRESULT_VIDEO_CHN_INVALID";
            status = GVFG_EINVAL;
            break;
        case GVFG_HRESULT_API_ERROR:
            name = "GVFG_HRESULT_API_ERROR";
            break;
        default:
            break;
        }

        const char *message = "Capture backend operation failed";
        switch (status)
        {
        case GVFG_EBUSY:
            message = "Capture backend is busy";
            break;
        case GVFG_ESTATE:
            message = "Capture backend is in an invalid state";
            break;
        case GVFG_EINVAL:
            message = "Capture backend rejected the channel";
            break;
        default:
            break;
        }
        error_state_.set_vendor(message,
    ...
    {
        {
            ...
            recovery_cv_.wait(stateLock, [this]()
                              { return !frame_held_ || !running_; });
        }
        ...
        gvfg_status_t status = from_vendor(GvfgStopCapture(context_, channel_), "GvfgStopCapture");
        if (status == GVFG_OK)
            status = from_vendor(GvfgStartCapture(context_, channel_), "GvfgStartCapture");
    ...
    gvfg_status_t GigabyteCaptureSession::wait_for_recovery(uint32_t timeoutMs)
    {
        ...
        { return !recovery_pending_ || !running_; };
    ...
    {
        error_state_.set(message ? message : "Capture backend operation rejected");
        return status;
    }
