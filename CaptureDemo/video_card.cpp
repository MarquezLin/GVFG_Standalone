...
int VideoCard::startCapture(ULONG channelIndex, int width, int height, const std::function<void(uchar *, int, int)> &cb)
{
    if (!is_initialized_)
    {
        return -1;
    ...
    DWORD frame_size = width * height * 2;
    if (ch.frame_buffer)
    {
    ...
    if (ch.interrupt_event == NULL)
    {
        ch.interrupt_event = CreateEvent(NULL, FALSE, FALSE, NULL);
        if (ch.interrupt_event == NULL)
        {
        ...
        eventReg.EventHandle = ch.interrupt_event;
        ...
        if (!ok)
        {
            qWarning() << "VideoCard: REGISTER_EVENT failed for ch" << channelIndex
                       << ", error:" << GetLastError();
            CloseHandle(ch.interrupt_event);
            ch.interrupt_event = NULL;
            ch.capturing = false;
            delete[] ch.frame_buffer;
            ch.frame_buffer = nullptr;
            return -1;
...
{
    ChannelState &ch = channels_[channelIndex];
    DWORD frame_size = ch.video_width * ch.video_height * 2;
    ULONG frame_index = 0;
    ...
    while (ch.capturing)
    {
        ...
        DWORD waitResult = WaitForSingleObject(ch.interrupt_event, 1000);
