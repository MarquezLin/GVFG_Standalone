...
int VideoCard::startCapture(ULONG channelIndex, int width, int height, UINT32 fourcc, const std::function<void(uchar *, int, int)> &cb)
{
    ...
    qDebug() << "VideoCard: startCapture ch" << channelIndex
             << "width:" << width
             << "height:" << height
             << "fourcc=0x" << Qt::hex << fourcc << Qt::dec
...
void VideoCard::captureThread(ULONG channelIndex)
{
    ChannelState &ch = channels_[channelIndex];
    ULONG frame_index = 0;
    ULONG dma_wait_timeout_count = 0;
    bool logged_first_dma = false;

    // Enable interrupt mask for this channel
    writeReg(INTERRUPT_BASE + IRQ_MASK_W1S_OFFSET, ch.irq_mask_bit);
    qDebug() << "VideoCard: captureThread started ch" << channelIndex
             << "irq_mask=0x" << Qt::hex << ch.irq_mask_bit << Qt::dec;
    ...
    while (ch.capturing)
    {
        ...
        if (waitResult != WAIT_OBJECT_0)
        {
            ...
            dma_wait_timeout_count++;
            if ((dma_wait_timeout_count % 3) == 0)
            {
                ULONG irq_pending = 0;
                ULONG irq_masked = 0;
                ULONG irq_mask = 0;
                ...
                ULONG lost_frame = 0;
                ULONG video_irq = 0;
                ULONG video_dma_irq = 0;
                readReg(INTERRUPT_BASE + IRQ_PENDING_STATUS_OFFSET, irq_pending);
                readReg(INTERRUPT_BASE + IRQ_MASKED_STATUS_OFFSET, irq_masked);
                ...
                readReg(ch.video_base + VIDEO_DMA_IRQ_STATUS_OFFSET, video_dma_irq);
                qDebug() << "VideoCard: waiting for DMA interrupt ch" << channelIndex
                         << "timeouts:" << dma_wait_timeout_count
                         << "irq_pending=0x" << Qt::hex << irq_pending
                         << "irq_masked=0x" << irq_masked
        ...
        dma_wait_timeout_count = 0;
        if (!logged_first_dma)
        {
            qDebug() << "VideoCard: first DMA interrupt received ch" << channelIndex;
            logged_first_dma = true;
        }
        ...
        if (getVideoDoneIndex(channelIndex, frame_index))
        {
            if (ch.frame_count < 5)
            {
                qDebug() << "VideoCard: done index ch" << channelIndex << ":" << frame_index;
            }
        ...
        if (ch.frame_count <= 5)
        {
            qDebug() << "VideoCard: getFrame ch" << channelIndex
                     << "bytes:" << ret
                     << "expected:" << frame_size;
        }
